...
class TimeBar extends Component {
	...
	render({ seekTime, onMouseMove }) {
		let containerHeight = isIE() ? 15 : 4;
		let seekBarStyle = {
				backgroundImage: (!isIE()) ? `-webkit-gradient(linear, left top, right top, color-stop(${seekTime /
					100}, #ff8a16), color-stop(${seekTime / 100}, #ffffff))`: null,
